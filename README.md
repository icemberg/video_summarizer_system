...
- ✂️ **Intelligent Audio Chunking** - Automatically slices large audio files (>40MB) to prevent memory crashes
...
3. **OpenAI Whisper** - Falls back to local Whisper transcription
   - *Memory Safe:* Automatically chunks files >40MB into 10-minute segments using `ffmpeg` before transcribing to prevent out-of-memory errors
...
│  │  ├─ ffmpeg (Optional): Slice into 10-min chunks (>40MB)   │
│  │  └─ Whisper: Transcribe to text (iterates chunks safely)  │
│  │                                                           │
...
## 🛡️ Enterprise-Grade Error Handling & Production Robustness

This application is hardened with a resilient backend designed to protect against API, memory, and validation failures:

### 1. Precision Rate-Limit (429) Detection 
- Detects HTTP 429 status codes and explicitly extracts `Retry-After` headers.
- Triggers internal exponential backoff with jitter to hide waiting periods from the user while keeping APIs safe.
- Rotates to the next available Google model automatically upon quota exhaustion.

### 2. Robust Content Guardrails
- Agent rotation triggers if Gemini returns an empty payload (`""`).
- Detects token size errors (`request too large`) and alerts the user when the context string generated by transcription should be chunked.

### 3. Regex-Driven URL Parsing
- Identifies and extracts precise video IDs from standard (`watch?v=`), shortlinks (`youtu.be/`), live streams (`/live/`), Shorts (`/shorts/`), and embeds (`/embed/`).

### 4. Memory-Safe Audio Chunking
- Transcribing long videos natively with OpenAI Whisper routinely crashes python with a `MemoryError`.
- The app checks `os.path.getsize()`. If it exceeds 40MB, it invokes `ffmpeg` to slice the audio into consecutive 10-minute `.m4a` segments, transcribing them one-by-one, forcing `gc.collect()` to completely bound memory limits.
